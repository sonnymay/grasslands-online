...
function preload() {
  ...
  this.load.image('rock_cliff_face_01', 'assets/decorations/rock_cliff_face_01.png?v=215');
  // Decorations
...
function buildDecorations(scene) {
  ...
  const cliffFaceKey = scene.textures.exists('rock_cliff_face_01') ? 'rock_cliff_face_01' : null;
  ...
  const addPromptInspiredLandmarks = () => {
    ...
    if (ruinsArchKey) {
      placeLandmarkDeco(ruinsArchKey, ruinX + 284, ruinY + 88, 228, {
        alignBottom: true,
        allowFlip: false,
        angle: 4,
        baseCluster: 0.10,
    ...
    if (mossyBoulderWideKey) {
      for (const b of [
        { dx: -32, dy: 148, h: 92, angle: 3 },
        { dx: 166, dy: 166, h: 74, angle: -7 },
      ]) {
        const x = ruinX + b.dx;
        const y = ruinY + b.dy;
        placeLandmarkDeco(mossyBoulderWideKey, x, y, b.h, {
          alignBottom: true,
          allowFlip: true,
          angle: b.angle,
          baseCluster: 0.14,
          contact: { width: b.h * 0.92, height: 20, yOffset: -6, alpha: 0.04, angle: b.angle, scuffs: 4 },
        });
        blockCells(x, y, 1);
      }
    }
    ...
    if (mossyBoulderWideKey) {
      for (const b of [
        { dx: -22, dy: 132, h: 86, angle: -6 },
        { dx: 104, dy: 132, h: 70, angle: 8 },
      ]) {
        const x = rockX + b.dx;
        const y = rockY + b.dy;
        placeLandmarkDeco(mossyBoulderWideKey, x, y, b.h, {
    ...
    if (cliffFaceKey) {
      placeLandmarkDeco(cliffFaceKey, rockX - 12, rockY + 190, 138, {
        alignBottom: true,
        allowFlip: true,
        angle: 3,
        baseCluster: 0.08,
        contact: { width: 220, height: 30, yOffset: -8, alpha: 0.045, angle: 3, scuffs: 6 },
      });
      blockCells(rockX - 12, rockY + 190, 2);
    }
    for (const p of [
  ...
  const addGrasslandsChokeLines = () => {
    ...
    for (const line of lines) {
      for (let i = 0; i < line.count; i++) {
        ...
        if (getZone(r, c) !== 'grasslands' || getCellType(r, c) !== 'grass') continue;
        const key = placed % 6 === 2 ? largePineKey : Phaser.Utils.Array.GetRandom(treeKeys);
        const tree = placeLandmarkDeco(key, x, y, key === largePineKey ? Phaser.Math.Between(292, 340) : Phaser.Math.Between(205, 260), {
          alignBottom: true,
          maxAngle: 3,
          baseCluster: 0.58,
        });
  ...
  const addAnchorCompositions = () => {
    ...
    const addAnchor = (tile_r, tile_c, zone, variant = 0) => {
      ...
      const dryCliffAnchor = cliffFaceKey && tileNoise(tile_r, tile_c, 1917) > 0.66;
      const anchor = {
        ...
        desert: { key: dryCliffAnchor ? cliffFaceKey : 'cactus_set', h: dryCliffAnchor ? Phaser.Math.Between(124, 150) : Phaser.Math.Between(130, 172), opts: { alignBottom: true, maxAngle: 4, blockRadius: dryCliffAnchor ? 2 : 1 } },
        ruins: { key: dryCliffAnchor ? cliffFaceKey : (ruinsPillarKeys.length ? Phaser.Utils.Array.GetRandom(ruinsPillarKeys) : Phaser.Utils.Array.GetRandom(rockKeys)), h: dryCliffAnchor ? Phaser.Math.Between(130, 158) : Phaser.Math.Between(136, 176), opts: { alignBottom: true, tint: dryCliffAnchor || ruinsPillarKeys.length ? null : ruinTint, maxAngle: 3, blockRadius: dryCliffAnchor ? 2 : 1 } },
